...
class ProductService {
  ...
  Future<void> _ensureDataDirectoryExists() async {
    ...
    if (!await directory.exists()) {
      print('ℹ️  [파일] "$dataDirectoryPath" 디렉토리가 없어 생성합니다.');
  ...
  Future<void> _saveProductsToFile(
    ...
    try {
      ...
      // 순위 추가
      final productsWithRanking = products.asMap().entries.map((entry) {
        return entry.value.copyWithRanking(entry.key + 1);
      }).toList();
      ...
      for (var element
        ...
        try {
          ...
          if (title.isNotEmpty && price > 0) {
            ...
            final basePrice =
                originalPrice ?? (price * 1.1).round(); // 원가가 없으면 추정
